...
In our previous journey ([What is the Local Clearance Minimum?](../LCM)),
we explored the fascinating world of navmesh generators and learned about the Local Clearance Minimum (LCM).
Remember, it defines the bottlenecks of a region, enabling us to merge navigation meshes for different agents into one
cohesive structure.
...
and now, it is time for us to explore its depths.

Our mission?
To integrate LCMs into the pipeline using Recast.
We will run tests, analyse results, and uncover the strengths and weaknesses of our new algorithm.

In Part 3, We will create a Navmesh generation method from scratch using the medial axis to detect LCMs.
The medial axis represents a shape's skeleton and is used to identify the tightest spots in the environment where LCMs
are needed.
...
In Part 4, we will use the generated LCMs for pathfinding, taking LCMs into account, which will be different from
standard methods.
...
2. **Triangle Clean-up & Voxelization**: Recast cleans up by removing all triangles with a steep slope.
...
4. **Region Partitioning**: The walkable space is divided into regions using watershed, monotone, and layer
   partitioning.
...
6. **Navmesh Generation**: Recast generates a polygon mesh from the simplified contour data, involving triangulating the
   contour vertices and removing duplicate vertices.
7. **Detailed Navmesh Generation**: The poly mesh and the height field are combined to create a more detailed mesh with
   accurate height approximations.

Once the navmesh is generated, it is handed over to Detour, the integrated pathfinding library.
Detour then uses this detailed navmesh for efficient game or application pathfinding.
...
For more details on the exact settings, check out the tests on
my [GitHub fork](https://github.com/JoranVandenbroucke/recastnavigation/tree/Thesis/Tests/RecastLCM) from Recast.
...
I will talk about the downside of our comparison method.
The results are only partially correct because we check whether the generated LCMs and Reference LCMs are roughly in the
same position.
As the comparison method does not know the shape of the environment, it does not know if an LCM is in a saddle segment.
This causes a generated LCM in the same saddle segment as the reference LCM to be marked as False Positive because their
position is too far away, even though the local clearance is the same.
Manually counting the True Positives and False Positives increases the precision and Accuracy of all environments, and
some environments even achieve a perfect score.
We will use auto-generated Precision and Recall values to draw our conclusion for this experiment.
...
Our new watershed implementation encounters issues with misaligned voxel data and input geometry, resulting in
unassigned grey cells.
Over-segmentation in non-axis-aligned areas leads to more False Positives and lower Precision due to multiple local
maxima.
Smaller cell sizes improve medial axis approximation but increase starting points for new regions, while larger sizes
cause heavily aliased edges.
This trade-off explains why there is an optimal cell size for PPrecision Mislabelled LCMs skew Recall and Precision
metrics,
which improve when manually corrected, achieving over 90% Recall in most environments with a Cell Size of 0.1.
...
Green lines are True Positive, Red lines are False Positive, and Black lines are Reference LCMs without a generated
counterpart.</sub>
...
The proposed method's Build Contours function is slower due to additional LCM extraction steps, but overall time is
comparable to the built-in method.
Statistical tests reveal significant differences in Build Regions and Build Contours times for City and Big City
environments, but total time differences are not always significant.
...
<sub id='F6'>Figure 6: Mean Total, Mean Build Regions, and Mean Build Contours (in milliseconds) between City (left) and Big City (right).
The p-value indicates the significance of the independent test.
The significance level is 0.05</sub>

### Part 2, Chapter 5: Conclusions

The current implementation is not recommended for general use due to low recall, which can cause agents to clip through
environments or enter regions they are too small for.
